...
export function Input({
  ...
  return (
    <label className={["flex w-full flex-col gap-2 text-sm", className].join(" ")}>
      {label ? <span className="font-medium text-[color:var(--ink)]">{label}</span> : null}
      <input
        id={id}
        {...props}
        className={[
          "w-full rounded-[var(--radius)] border border-[color:var(--border)] bg-white px-4 py-3 text-base text-[color:var(--ink)] shadow-[inset_0_1px_0_rgba(15,23,42,0.04)] outline-none transition",
