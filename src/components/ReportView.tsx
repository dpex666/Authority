import type { AuthorityReport } from "@/lib/authority/reportTypes";
import { Badge } from "./ui/Badge";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "./ui/Card";
import { Divider } from "./ui/Divider";
import { SectionTitle } from "./ui/SectionTitle";

function SeverityTag({ severity }: { severity: "High" | "Medium" | "Low" }) {
  const base = "rounded-full border px-2.5 py-1 text-xs font-medium";
  const map: Record<"High" | "Medium" | "Low", string> = {
    High: "border-[color:var(--error)]/20 bg-[color:var(--error)] text-white",
    Medium: "border-[color:var(--warning)]/30 bg-[color:var(--warning)] text-[color:var(--ink)]",
    Low: "border-[color:var(--border)] bg-[color:var(--surface2)] text-[color:var(--muted)]",
  };
  return <span className={`${base} ${map[severity]}`}>{severity}</span>;
}

function ProgressBar({ value }: { value: number }) {
  const v = Math.max(0, Math.min(100, value));
  return (
    <div className="mt-3 h-2 w-full rounded-full bg-[color:var(--surface2)] overflow-hidden">
      <div
        className="h-full rounded-full bg-gradient-to-r from-[color:var(--primary)] to-[color:var(--primary2)]"
        style={{ width: `${v}%` }}
      />
    </div>
  );
}

export default function ReportView({
  report,
  unlocked,
}: {
  report: AuthorityReport;
  unlocked: boolean;
}) {
  return (
    <Card id="report-root" className="bg-white/95 shadow-[var(--shadow-soft)]">
      <CardHeader className="space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div>
            <div className="text-sm text-[color:var(--muted)]">{report.profile.context}</div>
            <CardTitle className="mt-1 text-3xl sm:text-4xl">{report.profile.name}</CardTitle>
            <CardDescription className="mt-2">
              A premium-ready report snapshot with clear risks, actions, and timing.
            </CardDescription>
            <div className="mt-4 flex flex-wrap gap-2">
              <Badge>Status: {report.status}</Badge>
              <Badge>Confidence: {report.confidence}</Badge>
              <Badge>Updated: {report.updatedAtLabel}</Badge>
            </div>
          </div>

          <div className="rounded-[var(--radius)] border border-[color:var(--border)] bg-[color:var(--surface2)] px-5 py-4">
            <div className="text-xs tracking-[0.2em] text-[color:var(--muted)]">
              AUTHORITY INDEX
            </div>
            <div className="mt-1 text-4xl font-semibold leading-tight text-[color:var(--text)]">
              {report.overall}/100
            </div>
            <div className="mt-2 text-sm text-[color:var(--muted)]">
              Indicates how likely decisions + access will hold under pressure.
            </div>
          </div>
        </div>
      </CardHeader>

      {report.insight ? (
        <CardContent className="pt-0">
          <div className="rounded-[var(--radius)] border border-[color:var(--border)] bg-[color:var(--surface2)] px-5 py-4">
            <div className="text-xs tracking-[0.2em] text-[color:var(--muted)]">YOUR SNAPSHOT</div>

            <div className="mt-2 text-lg font-semibold text-[color:var(--text)]">
              {report.insight.headline}
            </div>

            <div className="mt-2 text-sm text-[color:var(--muted)]">
              {report.insight.summary}
            </div>

            <div className="mt-4 rounded-[var(--radius)] border border-[color:var(--border)] bg-white px-4 py-3">
              <div className="text-xs font-medium text-[color:var(--muted)]">
                If tomorrow happened
              </div>

              <div className="mt-2 space-y-1 text-sm text-[color:var(--muted)]">
                {report.insight.tomorrowSnapshot.map((line) => (
                  <div key={line}>• {line}</div>
                ))}
              </div>
            </div>
          </div>
        </CardContent>
      ) : null}

      <Divider className="my-8" />

      <CardContent className="pt-0">
        <div>
          <SectionTitle title="What breaks first" description="Top failure points, put simply." />
          <div className="mt-4 grid gap-3">
            {report.risks.map((r) => (
              <div
                key={r.id}
                className="rounded-[var(--radius)] border border-[color:var(--border)] bg-white px-5 py-4"
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="text-sm font-semibold text-[color:var(--text)]">{r.title}</div>
                  <div className="flex items-center gap-2">
                    <Badge>{r.timeframe}</Badge>
                    <SeverityTag severity={r.severity} />
                  </div>
                </div>
                <div className="mt-2 text-sm text-[color:var(--muted)]">
                  <span className="font-medium text-[color:var(--text)]">Why it matters:</span>{" "}
                  {r.whyItMatters}
                </div>
                <div className="mt-1 text-sm text-[color:var(--muted)]">
                  <span className="font-medium text-[color:var(--text)]">What to do:</span>{" "}
                  {r.whatToDo}
                </div>
              </div>
            ))}
          </div>
        </div>
      </CardContent>

      <Divider className="my-8" />

      <CardContent className="pt-0">
        <div>
          <SectionTitle title="Score breakdown" description="What moved the needle." />
          <div className="mt-4 grid gap-3">
            {report.pillars.map((p) => (
              <div
                key={p.pillar}
                className="rounded-[var(--radius)] border border-[color:var(--border)] bg-white px-5 py-4"
              >
                <div className="flex items-center justify-between">
                  <div className="font-medium text-[color:var(--text)]">{p.name}</div>
                  <Badge>{p.score}/100</Badge>
                </div>
                <ProgressBar value={p.score} />
                <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-[color:var(--muted)]">
                  {p.drivers.map((d, idx) => (
                    <li key={`${p.pillar}_${idx}`}>{d}</li>
                  ))}
                </ul>
              </div>
            ))}
          </div>
        </div>
      </CardContent>

      <Divider className="my-8" />

      <CardContent className="pt-0">
        <div>
          <SectionTitle
            title="Next 7 days plan"
            description="Clear actions with owners, effort, and timing."
          />
          <div className="mt-4 grid gap-3">
            {report.actions.map((a) => (
              <div
                key={a.id}
                className="rounded-[var(--radius)] border border-[color:var(--border)] bg-[color:var(--surface2)] px-5 py-4"
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="text-sm font-semibold text-[color:var(--text)]">{a.title}</div>
                  <div className="flex flex-wrap gap-2">
                    <Badge>Owner: {a.owner}</Badge>
                    <Badge>Effort: {a.effort}</Badge>
                    <Badge>Impact: {a.impact}</Badge>
                    <Badge>Due: {a.due}</Badge>
                  </div>
                </div>

                {a.template ? (
                  <div className="mt-3 rounded-[var(--radius)] border border-[color:var(--border)] bg-white px-4 py-3">
                    <div className="text-xs font-medium text-[color:var(--muted)]">
                      Message template
                    </div>
                    <div className="mt-1 text-sm text-[color:var(--muted)]">{a.template}</div>
                  </div>
                ) : null}
              </div>
            ))}
          </div>
        </div>
      </CardContent>

      <Divider className="my-8" />

      <CardContent className="pt-0">
        <div>
          <SectionTitle
            title="Authority Pack"
            description="Downloadable sheets designed for real-world use."
          />
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {report.pack.map((x) => {
              const isLocked = x.gated && !unlocked;
              return (
                <div
                  key={x.id}
                  className="relative overflow-hidden rounded-[var(--radius)] border border-[color:var(--border)] bg-white px-5 py-4"
                >
                  <div className="text-sm font-semibold text-[color:var(--text)]">{x.name}</div>
                  <div className="mt-1 text-sm text-[color:var(--muted)]">{x.desc}</div>
                  <div className="mt-3 flex gap-2">
                    <Badge>PDF</Badge>
                    <Badge>Printable</Badge>
                    <Badge>Shareable</Badge>
                  </div>

                  {isLocked ? (
                    <div className="absolute inset-0 flex items-center justify-center bg-white/80 backdrop-blur-[2px]">
                      <div className="rounded-[var(--radius)] border border-[color:var(--border)] bg-white px-4 py-3 text-sm text-[color:var(--muted)]">
                        Unlock to download
                      </div>
                    </div>
                  ) : null}
                </div>
              );
            })}
          </div>
        </div>
      </CardContent>

      <Divider className="my-8" />

      <CardContent className="pt-0">
        <div>
          <SectionTitle
            title="Conversation starter"
            description="A short script to make the chat easier."
          />
          <div className="mt-4">
            {!unlocked ? (
              <div className="rounded-[var(--radius)] border border-[color:var(--border)] bg-white px-5 py-4 text-sm text-[color:var(--muted)]">
                Unlock to view the conversation script.
              </div>
            ) : (
              <div className="rounded-[var(--radius)] border border-[color:var(--border)] bg-white px-5 py-4 text-sm text-[color:var(--muted)] space-y-2">
                {report.script.map((line) => (
                  <div key={line}>• {line}</div>
                ))}
              </div>
            )}
          </div>
        </div>

        <div className="mt-6 text-xs text-[color:var(--muted)]">
          This is a diagnostic tool, not legal advice.
        </div>
      </CardContent>
    </Card>
  );
}
